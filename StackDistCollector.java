...
public class StackDistCollector{
	...
	public Integer access(int address){
		...
		if(symbolTable.hasSymbol(address) == true){
			lastAccessTime = symbolTable.getRecord(address).getLastAccessTime();
        }
        if(lastAccessTime == -1){
		    pastAccesses.insert(this.time);
		    SymbolRecord inf = new SymbolRecord(address, this.time);
		    symbolTable.putRecord(address, inf);
		    this.time++;
			return null;
		}
		Scanner scan = new Scanner(pastAccesses.toStringAscendingOrder());
		while(scan.hasNext()){
			String next = scan.next();
			if(lastAccessTime == Integer.parseInt(next)){
				while(scan.hasNext()) {
					next = scan.next();
					LRU++;
				}
				pastAccesses.remove(lastAccessTime);
				symbolTable.getRecord(address).setLastAccessTime(this.time);
				pastAccesses.insert(this.time);
			}
		}
		this.time++;
	...
	public static void main(String[] args) throws IOException{
		...
		//if(collector.access(32)==null && collector.access(20)==2){
